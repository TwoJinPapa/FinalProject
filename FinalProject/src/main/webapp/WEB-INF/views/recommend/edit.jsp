		...
		<form action="update.do" method="post" id="edit-post-form"  enctype="multipart/form-data">
			...
			<div class="mb-3">
				<input type="text" class="form-control" name="recommend_Title"
					...
					placeholder="제목을 입력하세요." ${detail.memberNo != user ? 'readonly' : ''} />
			</div>
			
			<div class="location">
                <input type="text" class="form-control" value="${detail.restaurant_Name}" id="resName" name="restaurant_Name" placeholder="상호명" ${detail.memberNo != user ? 'readonly' : ''}/>
                <input type="text" class="form-control" value="${detail.restaurant_Addr}" id="resAddr" name="restaurant_Addr" placeholder="주소" ${detail.memberNo != user ? 'readonly' : ''}/>    
            </div>
			...
			<select class="form-select" name="recommend_Grade">
				<c:forEach var="i" begin="1" end="5">
					<option value="${i}" ${i == detail.recommend_Grade ? 'selected' : ''}>
						...
						<c:otherwise>
						<div id="selectpoint">
							<input class="form-select" name="category_Food" value="${detail.category_Food}" ${detail.memberNo != user ? 'readonly' : ''} />
							<input class="form-select" name="recommend_Grade" value="${detail.recommend_Grade}" ${detail.memberNo != user ? 'readonly' : ''} />	
						</div>
						</c:otherwise>
				</c:choose>
			<div class="row">
